...
const Post = ({ userdata, post, onClose, currentuserdata }) => {
      ...
      for (let i = 0; i < posttt.commentcount; i++) {
        ...
        try {
          ...
          if (commentSnapshot.exists()) {
            ...
            const likedByCurrentUser = commentData.likes.includes(
              currentuserdata.uid
            );
  ...
  const sendNotification = async (posttt) => {
    try {
  ...
  const getImageArray = () => {
    if (userdata && posttt) {
      ...
      for (let i = 0; i < posttt.mediaFiles.length; i++) {
        ...
        if (
          mediaFile.includes("jpeg") ||
          mediaFile.includes("png") ||
          mediaFile.includes("jpg") ||
          mediaFile.includes("gif")
        ) {
          mediaArray.push(
            <Image
              key={`image-${i}`}
          ...
          mediaFile.includes("mp4") ||
          mediaFile.includes("webm") ||
          mediaFile.includes("ogg")
        ) {
          mediaArray.push(
            <ReactPlayer
              key={`video-${i}`}
  ...
  const handlecommentlike = async (commentIndex) => {
    try {
      ...
      const com = q.data();
      if (
        !commentslikes[commentIndex] &&
        !com.likes.includes(currentuserdata.userName)
      ) {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 bg-opacity-70 overflow-y-auto flex justify-center transition-opacity duration-300 backdrop-blur-sm items-center ${
      ...
      {posttt && (
        <>
          ...
          <div className="bg-white dark:bg-gray-700 text-black dark:text-white bg-opacity-70 backdrop-blur-sm rounded-lg p-8 transition-transform duration-300 ">
            {!showcomments && commentslikes ? (
              <div className="font-rethink w-auto">
                <div className="card">
                  <div className="c1 flex justify-between">
                    <div className="c11 flex m-2">
                      ...
                      {userdata ? (
                        <>
                          <div className="name text-xl mt-2 mr-6 font-bold font-rethink">
                            {userdata.userName}
                          </div>
                          <div className="time mt-3">
                            {posttt &&
                              formatFirebaseTimestamp(
                                posttt.timestamp.toDate()
                              )}
                          </div>
                        </>
                      ) : null}
                  ...
                  <div className="comments flex font-bold">
                    ...
                    {posttt ? posttt.commentcount : 0} comments
                  </div>
                </div>
              </div>
            ) : (
              <>
                gfgfgf{showcomments}
                {
                  <div className="comments">
                    {posttt && comments.length > 0 ? (
                      comments.map((comment, index) => (
                        <div key={index} className="comment my-2 h-96">
                          <div className="flex justify-between">
                            <div className="commenter mr-3 text-sm font-bold">
                              {comment.userName}
                            </div>
                    ...
                    <button
                      className="bg-blue-500 hover:bg-blue-800 m-5 z-50"
