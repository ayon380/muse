    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            ref={modalRef}
            className="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-t-3xl p-4 pb-8 max-h-[80vh] overflow-y-auto"
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
          ...
          >
            <div className="pill-container flex justify-center mb-4">
              <div className="pill w-12 h-1.5 bg-gray-300 dark:bg-gray-600 rounded-full" />
            </div>

            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">Share Options</h2>
              <button
                onClick={closeModal}
            ...
            <div className="flex flex-col items-center">
              <p className="text-center mb-4 text-gray-700 dark:text-gray-300">
                Share <strong className="text-gray-900 dark:text-gray-100">{postdata?.caption}</strong> by{" "}
                <strong className="text-gray-900 dark:text-gray-100">{userName}</strong>
              </p>

              <motion.button
                onClick={handlesystemshare}
                className="bg-blue-500  text-white px-6 py-3 rounded-full shadow-md hover:bg-blue-600 focus:outline-none mb-6 flex items-center justify-center"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                <span>Share to System</span>
              ...
              <div className="grid grid-cols-3 gap-4 w-full">
                {chats?.map((chat) => (
                  <motion.button
                    ...
                    className={`bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 p-4 rounded-xl shadow-md hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none flex flex-col items-center ${
                      sharing[chat.roomid] ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                  >
                    <Image
                      src={chat.type === "p" ? (chat.participants[0] === userdata.uid ? usermetadata[chat.participants[1]]?.pfp : usermetadata[chat.participants[0]]?.pfp) : chat.pfp}
                      height={80}
                      width={80}
                    ...
                    <span className="text-sm text-center">
                      {sharing[chat.roomid] ? "Sharing.." : "Share to"} {chat.type === "p" ? (chat.participants[0] === userdata.uid ? usermetadata[chat.participants[1]]?.userName : usermetadata[chat.participants[0]]?.userName) : chat.title}
                    </span>
                  </motion.button>
                ))}
