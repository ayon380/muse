    ...
    return (
        <div className="lp w-1/3 ">
            <div className="bg-white z-50 pb-5 dark:bg-black h-full rounded-xl bg-clip-padding backdrop-filter backdrop-blur-3xl bg-opacity-40 shadow-2xl border-1 border-black lpo">
                {createpostopen && <CreatePost onClose={() => setcreatepostOpen(!createpostopen)} userdata={userdata} />}
                {userdata && (
                    ...
                    <div className="main1 h-full  flex flex-col justify-between items-center">
                        ...
                        {usage == "feed" && <div className="options flex flex-col  justify-evenly items-center flex-auto mt-10 md:mb-48">
                            <div className="explore">
                                <div className="text-2xl text-left font-bold  cursor-pointer" onClick={() => router.push('/feed')}>Feed</div>
                            </div>
                            <div className="explore">
                            ...
                            <div className="logout">
                                <div className="text-2xl font-bold cursor-pointer text-center" onClick={handleLogout}>Logout</div>
                            </div>
                        </div>}
                        {(currentuserdata || usage == "feed") && <div className="flex w-full justify-evenly -mt-6">
                            <div className="profile flex mt-2 cursor-pointer" onClick={() => (usage == "feed") ? router.push(`/${userdata.userName}`) : router.push('/feed')}>
                                <Image className="h-7 w-7 mr-2 rounded-full" src={usage == "slug" ? currentuserdata.pfp : userdata.pfp} alt="profile" width={100} height={100} />
                                <div className="name">
                                    {usage == "slug" ? currentuserdata.userName : userdata.userName}
                                </div>
                            </div>
                            {usage == "feed" &&
                                <button className="create btn px-6 py-2 font-rethink mb-4 " onClick={() => setcreatepostOpen(true)} disabled={
                                    createpostopen
                        ...
                        <div className="okedoesd flex w-full justify-evenly  text-sm">
                            <div className=" font-bold text-center cursor-pointer" onClick={() => router.push('/contactus')}>Contact Us</div>
                            <div className="about">
                                <div className=" font-bold text-center cursor-pointer" onClick={() => router.push('/about')}>About</div>
                            </div>

                        </div>
                        <div className="pl text-xs cursor-pointer" onClick={() => router.push('/releasenotes')}>
                            Muse v0.3 beta @NoFilter LLC 2024-2025
                        </div>
                    </div>
